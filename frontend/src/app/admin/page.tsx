...
type Article = { id: string; title: string; slug: string; content: string; tags: any[]; isVisible: boolean };
...
export default function AdminPage() {
  ...
  try {
    const res = await fetch(`${apiUrl}/upload`, { // <--- Endpoint nuevo
      method: 'POST',
      headers: { 
        'Authorization': `Bearer ${Cookies.get('token')}` // Si proteges la ruta, si no, quítalo
    ...
    if (activeTab === 'projects') {
      setProjectData(prev => ({ ...prev, [field]: data.url }));
    } 
  ...
  // Estilos de Inputs actualizados
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-end mb-12 border-b border-[#1a1a1a]/10 pb-6">
          ...
          <button 
            ...
            className="text-xs font-bold uppercase tracking-widest text-red-500 hover:text-red-700 hover:bg-red-50 px-4 py-2 transition-all border border-transparent hover:border-red-100"
          >
            Logout
          </button>
        </div>
        ...
        <div className="flex space-x-8 mb-8">
          <button 
            ...
            className={`pb-2 text-sm uppercase tracking-widest transition-all ${activeTab === 'projects' ? 'border-b-2 border-[#1a1a1a] font-bold text-[#1a1a1a]' : 'text-gray-400 hover:text-gray-600'}`}
          >
            Projects
          </button>
          <button 
            onClick={() => { setActiveTab('articles'); resetForm(); }} 
            className={`pb-2 text-sm uppercase tracking-widest transition-all ${activeTab === 'articles' ? 'border-b-2 border-[#1a1a1a] font-bold text-[#1a1a1a]' : 'text-gray-400 hover:text-gray-600'}`}
          >
            Articles
          </button>
        </div>
        ...
        <div className="bg-white border border-[#1a1a1a]/5 p-8 shadow-xl shadow-[#1a1a1a]/5 mb-12 relative">
          <div className="flex justify-between items-baseline mb-8 border-b border-gray-100 pb-4">
            ...
            {editingId && (
              <button onClick={resetForm} className="text-xs font-mono text-gray-400 hover:text-[#1a1a1a] border-b border-gray-300 hover:border-[#1a1a1a] transition-all">
                Cancel Editing
              </button>
            )}
          </div>
